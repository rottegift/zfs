...
ztest_dataset_create(char *dsname)
{
	 ...
	 * wrapping key.
	 *
	 * XXX: Creating encrypted datasets is currently
	 * disabled until https://github.com/zfsonlinux/zfs/issues/6526
	 * is fixed. Encrypted datasets disable compression
	 * on some block types (notably DMU_OT_DNODE) which
	 * triggers this issue much more frequently.
	 */
	rand = ztest_random(0);
	if (rand != 0) {
		nvlist_t *crypto_args = fnvlist_alloc();
