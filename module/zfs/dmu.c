...
void
dmu_assign_arcbuf_by_dbuf_impl(dmu_buf_t *handle, arc_buf_t *buf, dmu_tx_t *tx)
{
	dmu_buf_impl_t *db = (dmu_buf_impl_t *)handle;
 ...
 */
static void
dmu_assign_arcbuf_impl(dnode_t *dn, uint64_t offset, arc_buf_t *buf,
    dmu_tx_t *tx)
{
	dmu_buf_impl_t *db;
    ...
    dmu_tx_t *tx)
{
	...
	DB_DNODE_ENTER(dbuf);
	dmu_assign_arcbuf_impl(DB_DNODE(dbuf), offset, buf, tx);
	DB_DNODE_EXIT(dbuf);
}

void
dmu_assign_arcbuf(objset_t *os, uint64_t object, uint64_t offset,
    arc_buf_t *buf, dmu_tx_t *tx)
{
	dnode_t *dn;

	VERIFY0(dnode_hold(os, object, FTAG, &dn));
	dmu_assign_arcbuf_impl(dn, offset, buf, tx);
	dnode_rele(dn, FTAG);
}
