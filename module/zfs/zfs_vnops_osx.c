...
#endif
{
	...
	if (ap->a_vp) {
		int mmapped_write = 0;
		boolean_t mmapped = spl_ubc_is_mapped(ap->a_vp, &mmapped_write);
		if (mmapped) {
...
norwlock:
	...
	/*
	 * Empirically, we do not have to zero-fill the EOF page if
	 * usize is the same as zsize; in practice they are kept tightly
	 * in sync, and will be equal during pagein.
	 *
	 * Let's bleat about the (very rare) case when upl_offset is
	 * nonzero, as well as if zsize != usize.
	 */

	if (upl_offset > 0)
		VNOPS_OSX_STAT_BUMP(pagein_w_upl_offset);

	if (upl_offset > 0 || zp->z_size != ubc_getsize(vp)) {
		/*
...
start_tx:
	...
	if (error != 0) {
		...
		printf("ZFS: %s:%d: dmu_tx_assign error %d, in range [%u..%d]"
		    " (f_offset %lld write_size %lld) file %s fs %s\n",
		    __func__, __LINE__, error, upl_offset, size,
		    f_offset, write_size,
		    zp->z_name_cache,
		    vfs_statfs(zfsvfs->z_vfs)->f_mntfromname);
		zio_data_buf_free(safebuf, write_size);
	...
	if (!dmu_write_is_safe(zp, f_offset, f_offset + write_size)) {
		...
		if (new_blksz > zp->z_blksz) {
			...
			if (!dmu_write_is_safe(zp, f_offset, f_offset + write_size)) {
				pageout_op->state = "unsafe to dmu write";
				pageout_op->line = __LINE__;
...
#endif
{
	...
	pageout_op_t *pageout_op = kmem_zalloc(sizeof(pageout_op_t), KM_SLEEP);
	pageout_op->state = "init";
	pageout_op->func = __func__;
	pageout_op->line = __LINE__;
...
already_acquired_locks:
	...
	const off_t preserved_zsize = zp->z_size;
		...
		if (new_blksz > zp->z_blksz) {
			...
			dmu_tx_hold_sa(tx, zp->z_sa_hdl, B_FALSE);
			zfs_sa_upgrade_txholds(tx, zp);
			pov2h_dmu_tx_hold_write(tx, zp->z_id, 0, new_blksz);
			error = dmu_tx_assign(tx, TXG_WAIT);
			ASSERT3S(error, ==, 0);
			if (error != 0) {
				dmu_tx_abort(tx);
...
skip_lock_acquisition:
	...
	if (upl_pages_dismissed == pages_in_upl) {
		...
		const int len_of_tail = ap->a_size - start_of_tail;
		if (spl_ubc_is_mapped_writable(vp)) {
			printf("ZFS: %s:%d: (mapped writable) trimmable tail"
			    " [%d..%d] (len %d) zid %llu fs %s file %s\n",
			    __func__, __LINE__,
			    start_of_tail, start_of_tail + len_of_tail, len_of_tail,
			    zp->z_id, fsname, fname);
		}
		int abort_flags = 0;
		int abort_tail_ret = ubc_upl_abort_range(upl,
		    start_of_tail, len_of_tail, abort_flags);
		if (abort_tail_ret != KERN_SUCCESS
			...
			printf("ZFS: %s:%d ERROR %d aborting tail of UPL"
			    " [%d..%d] (len %d) a_f_offset %llu a_size %lu"
			...
			printf("ZFS: %s:%d: (ret %d) %d pages [%d..%d] (len %d)"
			    " trimmed from tail of %d page UPL"
			    " (valid pages in tail %d)[%lld..%lld] fs %s file %s"
			    " (mapped? %d write? %d)\n",
			    __func__, __LINE__, abort_tail_ret, upl_pages_dismissed,
			    start_of_tail, start_of_tail + len_of_tail, len_of_tail, pages_in_upl,
	...
	if (upl) {
		...
		if (pg_index < just_past_last_valid_pg) {
			printf("ZFS: %s:%d: ANOMALY SHORT ERROR: pg_index lower than expected (%llu v %llu)"
			    ...
			    " UPL file range %llu..%llu @ zid %llu fs %s fname%s\n", __func__, __LINE__,
		...
		if (error != 0) {
			printf("ZFS: %s:%d: ANOMALY error %d ERROR:"
			    " will not commit!"
			    " pg_index %lld @ UPL file range %llu..%llu @ zid %llu fs %s fname %s\n",
			    __func__, __LINE__, error,
			    pg_index, f_start_of_upl, f_end_of_upl,
			    zp->z_id, fsname, fname);
			commit = B_FALSE;
		}
		const int lowest_page_dismissed = pages_in_upl - upl_pages_dismissed;
		...
		if (commit == B_TRUE) {
			...
			int final_abort_ret = ubc_upl_abort_range(upl,
			    commit_from_page * PAGE_SIZE_64, abort_size, abort_flags);
	...
	if (upl) {
		...
		if (upl_pageout_done != KERN_SUCCESS) {
			printf("ZFS: %s:%d: upl_pageout_done abort ERROR %d"
			    " upl offset %llu upl_size %lu flags 0x%x"
			    " zid %llu fs %s file %s\n", __func__, __LINE__,
			    upl_pageout_done,
			    ap->a_f_offset, ap->a_size, ap->a_flags,
			    zp->z_id, fsname, fname);
			error = upl_pageout_done;
		} else {
			printf("ZFS: %s:%d: (pageout_done) upl != NULL, retired it,"
			    " upl offset %llu upl_size %lu flags 0x%x"
			    " zid %llu fs %s file %s\n", __func__, __LINE__,
			    ap->a_f_offset, ap->a_size, ap->a_flags,
			    zp->z_id, fsname, fname);
...
#endif
{
	...
	if (ISSET(ap->a_fflags, VM_PROT_WRITE)) {
        ...
        const rl_type_t rltype = (ISSET(ap->a_fflags, VM_PROT_WRITE))
	    ? RL_WRITER
	    : RL_READER;
...
zfs_vfsops_init(void)
{
	...
	tsd_create(&pageout_op_key, NULL);
