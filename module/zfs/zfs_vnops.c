    ...
    fill_holes_direction_t who_for)
{
	...
	int upl_flags = UPL_SET_LITE | UPL_FILE_IO | UPL_RET_ONLY_ABSENT;
...
fill_holes_in_range(vnode_t *vp, const off_t upl_file_offset, const size_t upl_size, fill_holes_direction_t who_for)
{
	...
	for (int i = 0; err == 0 && cur_upl_size > 0; i++) {
		...
		int uplcflags = UPL_FILE_IO | UPL_SET_LITE | UPL_RET_ONLY_ABSENT;
		...
		if (ioflag & FAPPEND) {
			...
			for ( ; ; ) {
				...
				if (((rlock->r_off & PAGE_MASK_64) == 0
				    ...
				    || (rlock->r_len == UINT64_MAX && rlock->r_off == 0)) {
					break;
				}
				...
				ASSERT3U(locked_off, >=, aligned_locked_off);
			...
			tsd_set(rl_key, rlock);
			ASSERT3U(trunc_page_64(zp->z_size), ==, rlock->r_off);
			woff = zp->z_size;
		} else {
			ASSERT3P(tsd_get(rl_key), ==, NULL);
