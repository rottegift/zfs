...
static vnops_stats_t vnops_stats = {
	...
	{ "zfs_write_cluster_copy_ok",                   KSTAT_DATA_UINT64 },
	{ "zfs_write_cluster_copy_complete",             KSTAT_DATA_UINT64 },
	{ "zfs_write_cluster_copy_bytes",                KSTAT_DATA_UINT64 },
	{ "zfs_write_cluster_copy_error",       	 KSTAT_DATA_UINT64 },
	{ "zfs_write_cluster_short_write",	         KSTAT_DATA_UINT64 },
    ...
    rl_t *rl)
{
	...
	for ( ; resid > 0; pg_index++) {
		...
		if (!upl_valid_page(pl, pg_index)) {
			 ...
			 * context
			 */
			printf("ZFS: %s:%d (note) non-valid page"
			    " at index %d of %d"
	...
	*bytes_to_copy = resid;
	...
	const hrtime_t ds_THRESHOLD = MSEC2NSEC(100);
	const hrtime_t cs_THRESHOLD = MSEC2NSEC(10);

	if (dt_final > ds_THRESHOLD) {
		printf("ZFS: %s:%d: very long UBC read time (> 100 ms) in nsec:"
	...
	} else if (dt_after_map > cs_THRESHOLD || dt_after_upl > cs_THRESHOLD) {
		printf("ZFS: %s:%d: long UBC read time (> 10 ms) in nsec:"
		    " dt_final %lld dt_after_map %lld dt_after_upl %lld off %lld sz %ld fs %s file %s\n",
		    __func__, __LINE__, dt_final, dt_after_map, dt_after_upl,
		    upl_file_offset, upl_size, fsname, fname);
	}
    ...
    off_t woff, int error)
{
	...
	for (int c = proj_chunks ; uio_resid(uio) > 0; c--) {
		 ...
		 */
		if (ioflag & FAPPEND) {
			const off_t tr_this_off = trunc_page_64(this_off);
			off_t rn_this_off = round_page_64(this_off);
		...
		if (zp->z_in_pager_op > 0) {
			printf("ZFS: %s:%d: see z_in_pager_op %d before cluster copy"
			    ...
			    uio_offset(uio), uio_offset(uio) + xfer_resid,
			    fsname, fname);
		}
	...
	error = dmu_tx_assign(tx, TXG_WAIT);
	if (error) {
		printf("ZFS: %s:%d: error %d from dmu_tx_assign fs %s file %s\n",
	 ...
	 * if we are at the end of the file, trim the last page
	 * so that everything past the EOF is zero-filled rather
	 * than random garbage.   This is important for any client
	 * who is mmapping the last page.
	 */
		...
		for ( ; ; ) {
			...
			if (rl != NULL) {
				...
				if (now > print_at) {
					...
					printf("ZFS: %s:%d: waiting %u seconds for (pseudo)"
					    " APPEND range lock,"
					    " target len %llu, zsize %llu, fs %s file %s\n",
					    __func__, __LINE__, secs, target_len,
